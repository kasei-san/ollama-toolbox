...
**Brave は独自インデックスを持つ**のも効いている。2023年に Bing の
サーバーサイド呼び出しを撤廃して「100% independence」を公表しており
（[Brave の告知](https://brave.com/blog/search-independence/)）、
Google / Bing のポリシー変更に巻き込まれにくい。
SearXNG 経由では結果の75%が `google cse` 由来だったので、この点は改善している。
（Google Fallback Mix のようなフォールバック機能は web UI 側の opt-in。
**API 側の挙動は未確認。**）

**なぜ brave / ddgs / searxng の3つに落ち着いたのか**、その経緯と外した仮説は
[issues/5](https://github.com/kasei-san/ollama-toolbox/issues/5) にある
...
#### `off` で成人向けが実際に返ることの確認（2026-08-11）

上の表は「値で結果が変わる」ことしか示さないので、`off` が実際に素通しなのかを別途確認した。
**Brave API・`safesearch=off`**、`count=20`、各クエリ1回:

| クエリ | `off` | `strict` |
|---|---|---|
| `free porn videos` | 20件中 **10件** | **0件** |
| `エロ漫画 サイト` | 20件中 **2件** | 4件中 0件 |
| `hentai` | 20件中 **4件** | 4件中 0件 |

件数は**成人向けドメインの数**。判定はホスト名の部分一致
（`xvideos` `pornhub` `xhamster` `xnxx` `dlsite` `fanza` `nhentai` `e-hentai` `rule34`）
という粗い基準なので、取りこぼしはある。

**言えるのは「`off` は素通しとして機能する」まで。**
測ったのは成人向けカテゴリだけで、**他の領域で絞られていないことは確認していない**。
検索結果は時期で変わるので、**判断を左右するなら測り直すこと。**

**`strict` でも 0 件にならないことがある**（上表の 4件）。完全な遮断は期待しないこと。
